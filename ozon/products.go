...
type ProductDetails struct {
	...
	Promotions []ProductDetailsPromotions `json:"promotions"`
}

type ProductDetailsPromotions struct {
	// 如果促销活动已启用，则为 true。
	IsEnabled bool `json:"is_enabled"`
	//
	//Default: "UNSPECIFIED"
	//Enum: "UNSPECIFIED" "REVIEWS_PROMO"
	//促销活动类型：
	//
	//UNSPECIFIED — 未知类型;
	//REVIEWS_PROMO — “评价积分”促销活动。
...
type ProductsImportPromotions struct {
	Type      string `json:"type"`      // REVIEWS_PROMO: “评价积分”促销活动
	Operation string `json:"operation"` // ENABLE — 启用， DISABLE — 禁用， UNKNOWN — 保持默认，不做修改。
}
...
type CreateOrUpdateProductItem struct {
	...
	Promotions []ProductsImportPromotions `json:"promotions"`
}
