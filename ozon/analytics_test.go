	...
	tests := []struct {
		...
		// Test ok
		{
			...
			&GetAnalyticsDataParams{
				DateFrom:  ozonCore.NewTimeFormat(time.Now().Add(time.Duration(30)*24*time.Hour), ozonCore.ShortDateLayout),
				DateTo:    ozonCore.NewTimeFormat(time.Now(), ozonCore.ShortDateLayout),
