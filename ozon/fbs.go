...
type ListUnprocessedShipmentsFilter struct {
	...
	CutoffFrom *ozonCore.TimeFormat `json:"cutoff_from,omitempty"`

	// Filter by the time by which the seller should pack the order. Period end.
	//
	// Format: YYYY-MM-DDThh: mm:ss. mcsZ. Example: 2020-03-18T07:34:50.359 Z
	CutoffTo *ozonCore.TimeFormat `json:"cutoff_to,omitempty"`

	// Minimum date when shipment should be handed over for delivery
	DeliveringDateFrom *ozonCore.TimeFormat `json:"delivering_date_from,omitempty"`
...
func (c FBS) PrintLabeling(ctx context.Context, params *PrintLabelingParams) (*PrintLabelingResponse, error) {
	...
	if err != nil {
		fmt.Println("FBS PrintLabeling 2:", err)
		return nil, err
	}
	...
	if errJson != nil {
		...
		for _, detail := range resp.Details {
			messageList = append(messageList, detail.Message)
		}
		return nil, errors.New(resp.Message + ";" + strings.Join(messageList, ";"))
	}
